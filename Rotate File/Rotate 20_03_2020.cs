...
public class Rotate : MonoBehaviour
{
    ...
    void PlutoModelStart()
    {
        ...
        r_Pluton = 39.482117f * 149597870700f; //В а.е. = 39.482117f //
        G = 6.67f * Mathf.Pow(10, -11);
        massSun = 1.9885f * Mathf.Pow(10, 30);
        massPluto = 1.303f * Mathf.Pow(10, 22);

        R_Pluton = (1 - 0.2488273f) * a_Pluton * 149597870700f;//
        h1_Pluton = -G * (massSun + massPluto);
        a1_Pluton = h1_Pluton / (R_Pluton * Mathf.Pow(4669.1f, 2)); //Vp = 29813979130.6f - старое значение
        //Debug.Log("a1_Pluton = " + a1_Pluton);
        vp_Pluton = (-2 * a1_Pluton * Mathf.PI * R_Pluton / (orbitalPeriod_Pluton * Mathf.Pow((-2 * a1_Pluton) - 1, 1.5f))) * 61f;//1726.6724904f * 61;
        Debug.Log("vp_Pluton = " + vp_Pluton);
        rSred_Pluton = 1.6621161561f; //rSred_Pluton = r_Pluton / R_Pluton;

        hi_Pluton = h;

        vt_Pluton = (vp_Pluton / rSred_Pluton);

        vtSred_Pluton = vt_Pluton / vp_Pluton;
    ...
    void EarthModelStart()
    {
        ...
        // Debug.Log("h1 = " + h1);
        a1 = h1 / (R * Mathf.Pow(30027, 2)); //-77.3030194662f
        //Debug.Log("a1 = " + a1);
        vp = (-2 * a1 * Mathf.PI * R / (orbitalPeriod * Mathf.Pow((-2 * a1) - 1, 1.5f))) * 61f;
        Debug.Log("vp = " + vp);
        //Debug.Log("vp = " + vp);
        rSred = r / R;
        ...
        vrSred = (Mathf.Sqrt(Mathf.Pow(a1 + 1, 2) - Mathf.Pow((a1 + 1) / rSred, 2))) * 61f;
