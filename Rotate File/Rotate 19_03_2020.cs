...
public class Rotate : MonoBehaviour
{
    ...
    void PlutoModelStart()
    {
        ...
        r_Pluton = 5.9064406f * Mathf.Pow(10, 12); //В а.е. = 39.482117f
        G = 6.67f * Mathf.Pow(10, -11);
        massSun = 1.9885f * Mathf.Pow(10, 30);
        massPluto = 1.303f * Mathf.Pow(10, 22);

        R_Pluton = (1 - 0.2488273f) * 5906440606.29f;//
        h1_Pluton = -G * (massSun + massPluto);
        a1_Pluton = h1_Pluton / (R_Pluton * Mathf.Pow(1726.6724904f, 2)); //Vp = 29813979130.6f - старое значение
        //Debug.Log("a1_Pluton = " + a1_Pluton);
        vp_Pluton = 1726.6724904f;
    ...
    void EarthModelStart()
    {
        ...
        // Debug.Log("h1 = " + h1);
        a1 = h1 / (R * Mathf.Pow(30280, 2)); //-77.3030194662f
        Debug.Log("a1 = " + a1);
        vp = ((-2 * a1 * Mathf.PI * R) / (orbitalPeriod * Mathf.Pow((-2 * a1) - 1, 1.5f)));
        Debug.Log("Начальное vp = " + vp);
        //Debug.Log("vp = " + vp);
        rSred = 1.01699524175f; // r / R
        ...
        e = (2 * Mathf.PI) / orbitalPeriod;
        Debug.Log("e = " + e);

        //Вычисление угла
        f = (vp / R) * (vtSred / rSred) * hi + e * Mathf.Pow(hi, 2) / 2;
        Debug.Log("Начальное f = " + f * 1000 * Mathf.Pow(10, 8));
        X1 = a * Mathf.Cos(f* Mathf.Pow(10, 8));
        //Debug.Log("X1_Pluton = " + X1_Pluton);

        Y1 = b * Mathf.Sin(f * Mathf.Pow(10, 8));
        //Debug.Log("Y1_Pluton = " + Y1_Pluton);
